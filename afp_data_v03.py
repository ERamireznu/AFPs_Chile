...
#v03: 22/10/25 adds UF values
import sys
import pandas as pd
import fondoA, fondoB, fondoC, fondoD, fondoE, lis_uf
...
global lis_ufmes
lis_ufmes = lis_uf.lis_uf_mes  #[(0, '01-04-2002', 16197.66), (21, '02-05-2002', 16257.01),...]
...
def gr_(event):
    def graph_mlines(lis_x, Mlis_y, lis_ufxy):
        ...
        #uf--------------\/
        uf_xvals = [x[0] for x in lis_ufxy]
        uf_yvals = [y[2] for y in lis_ufxy]
        plt.plot(uf_xvals, uf_yvals, label = f'valor UF', linewidth=0.5)
        #----------------/\
        plt.xlabel("dias")
    ...
    graph_mlines(lis_dates, [lis_valsA, lis_valsB, lis_valsC, lis_valsD, lis_valsE], lis_ufmes)

def gr_slog(event):
    def graph_mlines_slog(lis_x, Mlis_y, lis_ufxy):
        xvals = [x for x in range(len(lis_x))]
        plt.figure(figsize=(12,6))
        plt.grid(color='0.95')
        dic_fondos = {0:'A',1:'B',2:'C',3:'D',4:'E'}
        ...
        uf_xvals = [x[0] for x in lis_ufxy]
        uf_yvals = [y[2] for y in lis_ufxy]
        plt.semilogy(uf_xvals, uf_yvals, label = f'valor UF', linewidth=0.5)
        #----------------/\        
        plt.xlabel("dias")
        plt.ylabel("valor cuota")
    ...
    graph_mlines_slog(lis_dates, [lis_valsA, lis_valsB, lis_valsC, lis_valsD, lis_valsE], lis_ufmes)
